...
/*
    아래 코드에서 if() {} 이전에 선언된 num2 와
    if() {} 블럭 내부에 선언된 num2 변수는
    이름은 같지만 일반적인 프로그래밍 언어에서는
    전혀 다른 변수로 취급된다
    따라서 if() {} 내에서 num2 변수에 저장된값은
    if() {} 이 종료되면 사라지고
    if() {} 이후에 num2 변수의 값은 200이 되어야 한다
    JS 에서 var 키워드로 선언된 변수는 그렇지 않다
    이는 scope issue 라고 하며 매우 중요한 issue 이므로
    잘 살펴야 한다
*/
var num2 = 200;
if (num1 % 2 == 0) {
